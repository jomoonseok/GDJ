...
--       (1) CONCAT 함수의 인수(argument)는 2개만 지원
--       (2) A와 B와 C의 연결
--           ① CONCAT(A, B, C)         : 불가능
--           ② CONCAT(CONCAT(A, B), C) : 가능
SELECT
       FIRST_NAME || ' ' || LAST_NAME AS FULL_NAME
     , CONCAT(CONCAT(FIRST_NAME, ' '), LAST_NAME) AS FULL_NALE  
  FROM
       EMPLOYEES;
...
-- 6. 문자열 채우기(PADDING)
--    1) LPAD(칼럼, 전체폭, 채울문자) 왼쪽으로 채우기
--    2) RPAD(칼럼, 전체폭, 채울문자) 오른쪽으로 채우기
SELECT
...
-- 위에 함수들을 JAVA에서(DB를 꺼내서 JAVA로 가져가서 작업) 할 수 있지만 Oracle에서 작업하는 게 개발자의 소양이다.      

-- 데이터 분석을 가장 많이하는 빅데이터에서 가장 많이 사용하는 언어 1~3위 중에 SQL은 거의 대부분 들어간다.
