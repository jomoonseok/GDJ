...
SELECT 
  ...
  FROM 
       EMPLOYEES
 WHERE
       DEPARTMENT_ID IS NOT NULL; -- NULL이 사라지면 인덱스 사용가능 그러면 검색속도도 빨라지고 사용하기 좋아진다.  
...
-- JOIN 없이 EMPLOYEES 테이블만 이용하여 IMPLOYEE_ID와 DEPARTMENT_NAME 조회하기
SELECT
       EMPLOYEE_ID
     , DECODE(DEPARTMENT_ID
        ...
        , 50, 'Shipping'
        , 60, 'IT') AS 부서명 
        -- DB의 성능을 올리기 위해서 JOIN을 사용하지 않고 사람이 직접 입력
  FROM
       EMPLOYEES;
...
-- 퇴직금정산유무 : 근무개월수가 240개월 이상이면 '정산대상', 아니면 빈 문자열
SELECT
       EMPLOYEE_ID AS 사원번호
