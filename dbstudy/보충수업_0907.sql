...
-- WHERE ORDER_DATE NOT BETWEEN '14/07/04' AND '14/07/07';
 WHERE TO_DATE(ORDER_DATE) NOT BETWEEN TO_DATE('14/07/04', 'YY/MM/DD') AND TO_DATE('14/07/07', 'YY/MM/DD'); 
...
-- WHERE ORDER_DATE NOT BETWEEN '14/07/04' AND '14/07/07';
   AND TO_DATE(ORDER_DATE) NOT BETWEEN TO_DATE('14/07/04', 'YY/MM/DD') AND TO_DATE('14/07/07', 'YY/MM/DD');
...
SELECT C.CUSTOMER_NAME AS 고객명, SUM(B.PRICE * O.AMOUNT) AS 총구매액
  FROM CUSTOMER C INNER JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK B
    ON B.BOOK_ID = O.BOOK_ID
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME; -- 이름이 같은 사람을 구분해줘야 한다.
...
SELECT C.CUSTOMER_NAME AS 고객명
     ...
     , COUNT(ORDER_ID) AS 구매횟수 -- COUNT에는 고객정보가 포함되면 안 됨
...
SELECT C.CUSTOMER_NAME AS 고객명
 ...
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;
--HAVING COUNT(O.ORDER_ID) = 0; -- HAVING절에는 대부분 집계 함수가 온다.    
...
-- 12. 총구매액이 2~3위인 고객의 이름과 총구매액을 조회하시오.
