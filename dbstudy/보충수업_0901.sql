...
SELECT DISTINCT C.COUNTRY_ID, C.COUNTRY_NAME -- DISTINCT는 중복을 제거할 칼럼 앞에 적어준다.
  FROM COUNTRIES C INNER JOIN LOCATIONS L
    ON C.COUNTRY_ID = L.COUNTRY_ID
 WHERE L.LOCATION_ID BETWEEN 1000 AND 1500;
 
-- 2) 서브쿼리 / 기본적으로 조인으로 풀이하나 이런 방법도 있다.
...
-- 조건 : 근무중인사원수 = 최대사원수 / HAVING절, 집계함수를 이용한 조건은 HAVING절만 가능함
