...
--    2) WHERE절 : GROUP BY 이전에 처리되므로 가능한 모든 조건은 WHERE절에서 처리함 / 그룹화할 데이터를 미리 줄이기 위해서 (성능향상)
...
HAVING 
       DEPARTMENT_ID < 100;
-- WHERE절과 HAVING절에서 모두 동작한다면 WHERE절을 사용한 쿼리문을 사용한다.
...
SELECT
 ...
 GROUP BY
       DEPARTMENT_ID
HAVING
       AVG(SALARY) >= 10000; -- 그룹함수들이 조건으로 쓰일 때는 전부 HAVING절에 쓴다.
...
--    부서번호가 없는 사원은 제외하기
SELECT -- 그룹화 함수를 사용한 칼럼, GROUP BY에 있는 칼럼이 와야하는데 여러 개면 그중 하나만 와도 조회된다.
       DEPARTMENT_ID
     , JOB_ID
     , COUNT(*) AS 사원수
  FROM 
...
-- GROUP BY에 함수가 들어가면 함수 그대로 사용한다.
