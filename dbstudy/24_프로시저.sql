/*
    ...
    1. PROCEDURE / JAVA의 메소드
    2. 여러 개의 쿼리문을 한 번에 실행
       (이체 : UPDATE문 2개)
    3. 작성된 프로시저는 EXECUTE문으로 실행
       EXECUTE 프로시저();
    4. 형식
        CREATE [OR REPLACE] 프로시저_이름[(매개변수)] / [OR REPLACE] 오류가 있으면 새 프로시저로 바꿔준다.
...
-- 정의는 한 번 해두면 프로시저 객체에 저장된다.
-- 그러면 저장된 프로시저를 여러 번 호출할 수 있다.
...
CREATE OR REPLACE PROCEDURE PROC3(EMP_ID IN EMPLOYEES.EMPLOYEE_ID%TYPE) -- JAVA랑 반대로 이름을 적고 타입을 적는다.
IS
...
-- 프로시저 PROC4 호출
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE; -- 1
BEGIN     --┎>  2 / FNAME을 전달하는 게 아니라 FNAME으로 결과가 전달된다.
...
-- 프로시저 PROC5 정의
-- 사원번호가 있으면 FIRST_NAME을 출력 파라미터로 전달, 없으면 'NoName'을 출력 파라미터로 전달하기
...
-- 연습1. 입력 파라미터에 사원번호 전달 출력파라미터에 FIRST_NAME 반환하기
...
CREATE OR REPLACE PROCEDURE BUY_PROC(C_NO IN CUSTOMER.CUST_NO%TYPE, P_CODE IN PRODUCT.PROD_CODE%TYPE, BUY_AMT IN BUY.BUY_AMOUNT%TYPE)
IS
BEGIN
--    1) 구매 테이블에 구매 내역을 INSERT 한다.
