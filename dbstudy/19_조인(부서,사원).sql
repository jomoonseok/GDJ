...
/*
    ...
    3. 조회할 테이블들은 관계를 줄 수 있어야 한다.
    4. 종류
        1) 크로스 조인 : 카테젼 곱, 각 테이블의 모든 행을 조인
            - CROSS JOIN
            - 조인 조건이 없으면 됨
            - 많은 행을 순식간에 만들 수 있음(기초데이터 작성요)
            - 조인 조건을 잘못 지정한 경우
        2) 내부 조인
            - INNER JOIN
...
-- 조인할 때는 별명을 써서 어느 테이블의 칼럼인지 명시해준다.
...
-- 모두포함시킬 사원테이블을 OUTER JOIN의 왼쪽/오른쪽에 두었느냐에 따라
-- 왼쪽 외부 조인/오른쪽 외부 조인으로 구분함

-- DRIVE/DRIVEN 테이블이 잘못 지정된 조인
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM EMPLOYEE E LEFT OUTER JOIN DEPARTMENT D -- 왼쪽에 EMPLOYEE 테이블은모두조회
