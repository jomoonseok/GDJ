...
-- 레코드 : 필드의 모임
-- DB에서 레코드란? 행(ROW)
-- 레코드 변수 : 여러 칼럼을 동시에 저장하는 변수(JAVA의 CLASS)
...
BEGIN
    ...
    ELSE
        RESULT := '저연봉';
    END IF;
    DBMS_OUTPUT.PUT_LINE(V_SALARY || '는 ' || RESULT || '입니다.');
END;
...
-- 2) EMPLOOYEES 테이블의 모든 사원의 FIRST_NAME, LAST_NAME 조회
-- FIRST_NAME과 LAST_NAME을 레코드 변수에 저장하고 해당 값을 조회
-- 레코드 변수는 사원 한 명의 정보만 저장할 수 있으므로 한 명씩 저장 후 출력하는 방식
-- 사원번호는 100 ~ 206 값을 가짐
DECLARE
    -- 참조 변수 선언
...
BEGIN
    ...
    WHILE EMP_NO <= 206 LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(PULL_NAME.FNAME || ' ' || PULL_NAME.LNAME);
        EMP_NO := EMP_NO + 1;
    END LOOP;
END;
...
BEGIN
    ...
    END LOOP;
    AVERAGE := TOTAL / CNT;
    DBMS_OUTPUT.PUT_LINE('연봉합계: ' || TOTAL || ' 연봉평균: ' || AVERAGE);
END;
...
/*
    EXCEPTION
        ...
        WHEN 예외 THEN
            예외처리
        WHEN OTHERS THEN -- OTHERS 모든 예외를 처리할 수 있다 / JAVA의 EXCEPTION 클래스
            예외처리
*/
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
...
EXCEPTION
    ...
    WHEN OTHERS THEN 
        DBMS_OUTPUT.PUT_LINE('예외코드 ' || SQLCODE); -- SQLCODE 예외코드를 찍어주는 키워드
        DBMS_OUTPUT.PUT_LINE('예외메시지 ' || SQLERRM); -- SQLERRM 예외메시지를 찍어주는 키워드 
END;
