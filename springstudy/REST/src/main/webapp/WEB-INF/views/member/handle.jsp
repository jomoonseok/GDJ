			...
			if(confirm('선택한 회원을 모두 삭제할까요?')){
				...
				for(let i = 0; i < $('.check_one').length; i++){
					if( $($('.check_one')[i]).is(':checked')){
						memberNoList += $($('.check_one')[i]).val() + ','; // 3번 1번 선택시 3, 1, (마지막 콤마 있음을 주의)
					}
				}
				memberNoList = memberNoList.substr(0, memberNoList.length -1); // 인덱스 0 ~ (길이 - 1)번 째 글자 가져오기
				$.ajax({
					type: 'delete',
