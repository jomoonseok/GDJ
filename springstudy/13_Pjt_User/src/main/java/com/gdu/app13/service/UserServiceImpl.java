...
public class UserServiceImpl implements UserService {
	...
	public Map<String, Object> sendAuthCode(String email) {
		...
		// 인증코드 만들기
		String authCode = securityUtil.getAuthCode(6);             // 수동으로 만든 코드
		// String authCode = securityUtil.generateRandomString(6); // 스프링이 만들어준 코드
	...
	public void join(HttpServletRequest request, HttpServletResponse response) {
		...
		if(!location.isEmpty() && promotion.isEmpty()) { // name 속성으로 넘어오는 값은 없을 때 null이 아닌 공백으로 온다.
			agreeCode = 1; // 필수 + 위치
	...
	public void retire(HttpServletRequest request, HttpServletResponse response) {
		...
		// 탈퇴할 회원의 userNo, id, joinDate는 session에 loginUser에 저장되어 있다.
		...
		try {
			...
			if(deleteResult > 0 && insertResult > 0) {
				...
				// session에 있는 정보는 DB에 저장된 정보가 아닌 서버 메모리에 저장된 정보이다.
	...
	public void keepLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		*/
		...
		if(keepLogin != null) {
			...
			// session_id
			String sessionId = request.getSession().getId(); // session의 아이디 값가져오기
	...
	public void logout(HttpServletRequest request, HttpServletResponse response) {
		...
		// 로그인 유지 풀기
		/*
		 * Cookie[] cookieList = request.getCookies(); // 쿠키를 전부 가지고와서 반복문으로 처리
	...
	public String getNaverLoginApiURL(HttpServletRequest request) {
		...
		String clientId = "ymBUOS66J0o3pUpDZezX";//애플리케이션 클라이언트 아이디값";
	...
	public String getNaverLoginToken(HttpServletRequest request) {
		...
		String clientId = "ymBUOS66J0o3pUpDZezX";//애플리케이션 클라이언트 아이디값";
	    String clientSecret = "8toAWv3hNm";//애플리케이션 클라이언트 시크릿값";
