...
public class UploadServiceImpl implements UploadService {
	...
	public void save(MultipartHttpServletRequest multipartRequest, HttpServletResponse response) {
		...
		// 1/제목/내용/1121/1121 / UPLOAD의 UPLOAD_NO를
		
		// 1/STORAGE/APPLE/APPLE/0/1 / ATTACH의 UPLOAD_NO로 넘겨줘야한다.
		// 2/STORAGE/BANANA/BANANA/0/1
		// 3/STORAGE/CHERRY/CHERRY/0/1
	...
	@Override
	public ResponseEntity<Resource> download(String userAgent, int attachNo) { // ResponseEntity<Resource> 페이지 바꾸지않고 값만 반환
		...
		try {
			...
			if(userAgent.contains("Trident")) {
				origin = URLEncoder.encode(origin, "UTF-8").replaceAll("\\+", " "); // +는 정규식에서 역할이 있음
			}
