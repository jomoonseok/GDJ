...
public class MyController2 {
	...
	/*
		...
		2. redirect
		return "redirect:/board/detail";  // mapping
		URL Mapping값이 /board/detail인 새로운 요청으로 redirect 하시오.
	*/
	
	// ※ 파라미터를 어떻게 처리할지 통일하지 않으면 불편한 일이 생길 수 있다.
	
	// <a href="${contextPath}/board/detail1?title=공지사항&hit=10">
	@GetMapping("detail1")
	public String detail1(HttpServletRequest request) { // request
	...
	@GetMapping("detail2")
	public String detail2(String title, int hit, Model model) { // requestParam
	...
	/*
		...
		메소드          addAttribute()	   addFlashAttribute() / addAttribute()는 forward처럼 동작하므로 사용하지 말 것
		
	*/
	
	// <a href="${contextPath}/board/detail3?title=공지사항&hit=10">
	@GetMapping("detail3")
	public String detail3(Board board  // object 
