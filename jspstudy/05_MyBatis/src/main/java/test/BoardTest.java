...
/*
 	...
 	3. 프로젝트의 Build Path에서 JUnit 라이브러리를 추고하고 사용한다.
 	4. 테스트 수행
 		프로젝트 실행 - Run - JUnit
 	5. 주요 애너테이션
...
public class BoardTest {
	...
	@Test
	public void 상세조회테스트() {
		// 7번 게시글의 제목이 '공지'면 테스트 성공, 아니면 실패
		/*
		Board board = BoardDao.getInstance().selectBoardByNo(7);
		assertEquals("공지", board.getTitle());
		*/
		// 7번 게시글을 가진 게시글이 있으면 테스트 성공, 아니면 실패
		Board board = BoardDao.getInstance().selectBoardByNo(7);
	...
	public void 게시글수정테스트() {
		...
		board.setBoardNo(11); // 게시글삽입테스트 결과로 삽인된 게시글번호
