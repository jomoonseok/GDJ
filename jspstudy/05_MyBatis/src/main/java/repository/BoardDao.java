...
public class BoardDao {
	...
	public List<Board> selectAllBoards(){
		SqlSession ss = factory.openSession(); // SELECT(커밋이 필요 없는 경우)        ┌> mapper의 namespace 속성 값 ┌> select의 id 속성 값
	...
	public int insertBoard(Board board) {
		...
		int result = ss.insert("mybatis.mapper.board.insertBoard", board); // board를 파라미터로 전달 / board.xml 파일 INSERT문에 VALUES 값으로 #{title}, #{content}가 없으면 안 됨
		if(result > 0) {
			ss.commit();
		}
		ss.close();
