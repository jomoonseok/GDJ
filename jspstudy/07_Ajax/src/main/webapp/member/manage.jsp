...
<script>
	$(document).ready(function(){
		// ajax 통신은 요청과 응답이 동일한 곳에서 이루어진다.
		fn_init();
		fn_getAllMembers();
		fn_getMember();
		fn_registration();
		...
		$.ajax({
			...
			success : function(resData) {
				...
				$.each(resData.members, function(i, member){
					...
					tr += '<td><input type="button" value="조회" class="btn_primary btn_detail"><input type="hidden" value="' + member.memberNo + '"><input type="button" value="삭제" class="btn_primary btn_remove"></td>';
					tr += '</tr>';
	...
	function fn_registration() {
		...
		$('#btn_add').click(function(){
			...
			$.ajax({
				...
				data : $('#frm_member').serialize(), // form의 모든 입력 요소를 파라미터로 변환해주는 jquery 메소드
				/* 응답 */
				dataType : 'json',
				// 정상 응답
	...
	function fn_modify() {
		...
		$('#btn_modify').click(function(){
			...
			$.ajax({
				/* 요청 */
				type : 'post', // insert와 update는 post로 요청
