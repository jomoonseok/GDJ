...
// @WebServlet({"/today.do", "/now.do", "/adder.do"}) 이 방법은 url맵핑이 새로 생길때마다 url맵핑을 추가해줘야한다.
...
public class MyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
						// Today의 request와 TodayService의 request는 서로 완전히 같다.
		// 요청 & 응답 인코딩
		...
		// 선택한 Modle의 실행
		if(myService != null) {
			actionForward = myService.execute(request, response); // 더 이상 doGet의 request, response를 처리하지 않고 TodayService 또는 NowService의 request, response를 처리한다.
		}
