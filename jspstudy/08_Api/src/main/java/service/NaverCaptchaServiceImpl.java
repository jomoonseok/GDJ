...
public class NaverCaptchaServiceImpl implements NaverCaptchaService {
	...
	// field
	private final String CLIENT_ID = "ymBUOS66J0o3pUpDZezX";
	private final String CLIENT_SECRET = "8toAWv3hNm";
	...
	public Map<String, String> getCaptchaImage(HttpServletRequest request, String key) {
		...
		try {
			...
			if (con.getResponseCode() == 200) { // 200 : HttpConnection_HTTP_OK
				...
				BufferedInputStream in = new BufferedInputStream(con.getInputStream()); // 성공하면 이미지가 넘어온다.
			...
			// 응답이 실패하면 텍스트 형식으로 응답
				...
				BufferedReader reader = new BufferedReader(new InputStreamReader(con.getErrorStream())); // 실패하면 텍스트가 넘어온다.
				StringBuilder sb = new StringBuilder();
				String line;
				while ((line = reader.readLine()) != null) {
					sb.append(line);
	...
	@Override
	public void refreshChptcha(HttpServletRequest request, HttpServletResponse response) {
