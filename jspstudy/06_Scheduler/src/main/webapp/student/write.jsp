...
<script>
	...
	$(document).ready(function(){
		...
		$('#frm_write').submit(function(event){
			...
			var eng = $('#eng');
			var math = $('#math');
			if(kor.val() == '' || inNaN(kor.val()) || kor.val() < 0 || kor.var() > 100) {
				alert('국어 점수를 확인하세요.');
				kor.focus();
				event.preventDefault();
				return;
			}
			else if(eng.val() == '' || inNaN(eng.val()) || eng.val() < 0 || eng.var() > 100) {
				alert('영어 점수를 확인하세요.');
				eng.focus();
				event.preventDefault();
				return;
			}
			else if(math.val() == '' || inNaN(math.val()) || math.val() < 0 || math.var() > 100) {
				alert('수학 점수를 확인하세요.');
				math.focus();
				event.preventDefault();
				return;
