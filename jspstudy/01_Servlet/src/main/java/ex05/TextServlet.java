...
public class TextServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 응답
			response.setContentType("text/plain; charset=UTF-8"); // 단순 텍스트의 MIME-TYPE / client의 dataType : 'text'와 맞추기
			
			PrintWriter out = response.getWriter();
			out.println("이름은 " + name + "이고, 나이는 " + age + "살입니다."); // client의 resData에 전달된다.
			out.close();
			...
			PrintWriter out = response.getWriter();
			out.println("예외 발생! 파라미터 age는 정수입니다."); // 개발자가 임의로 작성한 response 텍스트
			out.close();
