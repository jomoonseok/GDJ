...
public class PapagoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			con.setDoOutput(true); // post 방식으로 요청할 때 같이 쓰는 코드(보낼게 있다는 뜻) // 포스트 방식은 파라미터가 주소에 붙지않고 본문에 붙여서 보낸다.
			// 요청 파라미터 보내기
			String params = "source=" + source + "&target=" + target + "&text=" + text;
			OutputStream outputStream = con.getOutputStream(); // 바이트는 스트링을 못 보내서 바이트를 스트링으로 바꾼다.
