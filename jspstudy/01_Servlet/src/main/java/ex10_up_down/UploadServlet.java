...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		//     3) 실무에서는 한 곳에 모이지 않도록 첨부 파일의 경로를 매번 바꿔 줄 필요가 있음 / ip주소나 날짜를 기준으로
		String realPath = getServletContext().getRealPath("upload"); // src/main/webapp(web root) 아래 upload 디렉터리
		
		// 3. 업로드할 디렉터리 생성
		File dir = new File(realPath);
		if(dir.exists() == false) {
			dir.mkdirs(); // 경로안에 경로가 들어갈 때 mkdirs를 사용해야 하는데 그냥 mkdirs만 사용하자
		}
		...
		MultipartRequest multipartRequest = new MultipartRequest(
				...
				/* 최대 크기 */          1024 * 1024 * 10,
				/* 인코딩 */             "UTF-8",
				/* 파일명 중복 처리 */   new DefaultFileRenamePolicy() // 파일이름이 똑같으면 파일이름을 바꿔주는 정책 // 기본정책은 파일이름이 같으면 번호를 붙여준다.
				);
		
		// 5. 업로드 결과
		//    일반 리퀘스트인 HttpServletRequest는 사용할 수 없음 // HttpServletRequest를 재료로 MultipartRequest를 생성함
