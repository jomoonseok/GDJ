	...
	public static void main(String[] args) {
		...
		while(unit1.isAlive() && unit2.isAlive()) {  // while(unit1.getEnergy() > 0 && unit2.getEnergy() > 0) {} 
			if(myTurn) {
				...
				unit1.attack(unit2); // unit1이 unit2를 공격한다. (탱크는 10% 확률로 상대를 한 번에 죽인다.)
			} else {
				System.out.println(unit2.getName() + "의 공격!");
				unit2.attack(unit1); // unit2가 unit1을 공격한다. (마린은 40% 확률로 상대를 한 번에 죽인다.)
			}
		...
		// 승자 확인
		if(unit1.isAlive()) {
			System.out.println(unit1.getName() + "의 승리! " + unit1.getEnergy());
		} else {
			System.out.println(unit2.getName() + "의 승리! " + unit2.getEnergy());
		}
