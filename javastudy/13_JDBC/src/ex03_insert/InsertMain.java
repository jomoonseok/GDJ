...
public class InsertMain {
	...
	public static void main(String[] args) {
		...
		// 2. Board 클래스 타입 객체에 입력 받은 정보 저장하기
		...
		try {
			...
			String url = "jdbc:oracle:thin:@localhost:1521:xe"; // 종류와 버전을 알면 구글링으로 알아낼 수 있다.
			String user = "SCOTT";
			String password = "TIGER";
			con = DriverManager.getConnection(url, user, password);
			
			// 쿼리문 작성(변수 처리할 부분은 ?로 처리)                                                        // ? : 변수
			String sql = "INSERT INTO BOARD(BOARD_NO, TITLE, CONTENT, HIT, CREATE_DATE) VALUES(BOARD_SEQ.NEXTVAL, ?, ?, 0, SYSDATE)";

			// PreparedStatement 객체 생성 / 보안을위해 사용
 ...
 * 
 * try { System.out.print("제목 입력 >>> "); String title = sc.nextLine();
 ...
 * 
 * String sql =
 * "INSERT INTO BOARD(BOARD_NO, TITLE, CONTENT, HIT, CREATE_DATE) VALUES(BOARD_SEQ.NEXTVAL, ?, ?, 0, SYSDATE)"
 * ;
 * 
 * ps = con.prepareStatement(sql);
 * 
 * ps.setString(1, board.getTitle()); ps.setString(2, board.getContent());
 * 
 * int result = ps.executeUpdate();
 * 
