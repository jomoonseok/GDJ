...
public class SelectOneMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 쿼리문 실행
			rs = ps.executeQuery(); // rs는 next() 메소드를 사용하기 전까지 아무것도 안 한다.
			
			// 조회 결과를 행 단위로 스캔하는 rs 객체
			// rs.next() 메소드 호출로 조회 결과를 스캔할 수 있음
			// rs.next() 메소드 호출 1건 = 1행 스캔
			// re.next() 메소드는 스캔 성공 시 ture, 스캔 실패 시 false 반환
			...
			if(rs.next()) { // 스캔 성공하면(조회 결과가 있으면)
				/*
					...
					|     2    | 공지  |  내용2  |  0  |   22/09/07  | <= rs.next() 호출 시 rs 포인터
					...
					2. 칼럼의 순서(번호) / 칼럼의 순서와 타입을 정확히 알고있다면 사용
						1) rs.getInt(1)
						2) rs.getString(2)
						3) rs.getString(3)
						4) rs.getInt(4)
