...
public class Main2_JSON {
	...
	public static void m2() {
		...
		JSONObject obj2 = obj.getJSONObject("response"); // 가져올 property value에 따라서 메소드가 달라진다.
	...
	public static String m3() {
		...
		// postman
		// serviceKey 값은 인코딩된 값으로 보내기 
		// 탭중에서 Headers에서 key는 content-Type을 주고 value는 application/json; charset=UTF-8 준다.		
	...
	public static void m6() {
		...
		JSONArray columns = header.getJSONArray("columns"); // 배열이라 순서대로 저장됨

		JSONObject body = obj.getJSONObject("body");

		JSONArray items = body.getJSONArray("items");

		JSONObject item = items.getJSONObject(0); // 객체는 순서대로 저장 안 됨

		String[] p = { "trarNo", "mainTrarNm", "ctprvnCd", "ctprvnNm", "signguCd", "signguNm", "trarArea", "coordNum",
				"coords", "stdrDt" };

		Map<String, Object> map = new HashMap<String, Object>();

		for (int i = 0; i < columns.length(); i++) {
			map.put(columns.getString(i), item.get(p[i])); // Object 형태로 받아서 get만으로 진행가능
		}
	...
	public static void m8() {
		...
		for (int i = 0; i < dataList.length(); i++) {
			...
			System.out.println(
					weather.getInt("hour") + "시 : " + weather.getInt("temp") + "도, " + weather.getString("wfKor"));
		}
