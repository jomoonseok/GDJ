...
public class Main {
	...
	public static void m3() {
		...
		// 4. 형식
		//    try(스트림 생성) { - 이렇게 스트림을 생성하면 finally를 안 적어도 된다.
		//        코드
		//    } catch(Exception e) {
		//        e.printStackTrace();
		//    }
	...
	public static void m5() {
		...
		// FileWriter는 느리기 때문에
		// 빠른 속도가 필요한 경우 BufferedWriter를 추가해서 함게 사용한다.
		...
		try {
			...
			try {
				// 메인 스트림은 닫을 필요가 없음(보조 스트림을 닫으면 자동으로 메인 스트림이 닫힘)
				if(bw != null) {
					bw.close();
				}
			} catch(IOException e) {
	...
	public static void m6() {
		...
		// println()을 쓰기 위해서 printWriter를 사용하는 경우가 많다.
		// client <- server 
		//       stream : in / out
		//       inputStream(byte) -> inputStreamReader(char)
