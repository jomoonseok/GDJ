...
public class Main {
	...
	public static void m1() {
		...
		// C:\\storage\\trac1.txt 파일을
		// 1글자씩 읽어서
		// C:\\storage\\prac1_capy.txt 파일로 보내기
		...
		File cpy = new File("C:\\storage", "prac1_capy.txt");
		...
		/*
		 * File file = new File("C:\\storage", "prac1.txt");
		 * 
		 * long startTimestamp = System.currentTimeMillis();
		 * 
		 * try(FileReader fr = new FileReader(file)){
		 * 
		 * int c; StringBuilder sb = new StringBuilder(); while((c = fr.read()) != -1) {
		 * sb.append((char)c); } String str = sb.toString(); try(FileWriter fw = new
		 * FileWriter(new File("C:\\storage", "prac1_capy.txt"))){ fw.write(str);
